...
for t_group_fit in groups:
    for t_group_test in groups:
        ...
        print(f'Training {t_group_fit} and testing {t_group_test}')
        to_train = final_df.query(
            f"SO == '{t_group_fit}'")[markers + ['MR', 'Subject']]
        X_train = to_train[markers].values
        to_test = final_df.query(
            f"SO == '{t_group_test}'")[markers + ['MR', 'Subject']]
        ...
        print(f'Total time for {t_group_fit} - {t_group_test} was {elapsed}')
        cross_so_df = pd.concat(cross_so_results)
        cross_so_df.to_csv(
            f'../stats/results_decoding_balanced_{period}_cross_so.csv',
            sep=';')
