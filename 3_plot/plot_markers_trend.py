...
from sext import pageTest  # type: ignore
...
if subjects is not None:
    ...
    found_subjects = final_df['Subject'].unique()
    missing_subjects = [x for x in subjects if x not in found_subjects]
...
freqs = ['Alpha' if 'alpha' in x else 'Theta'
         for x in first_df['Marker'].values]  # type: ignore
first_df['Freq'] = freqs
first_df['Marker'] = [
    x.split('/')[-2]for x in first_df['Marker'].values]  # type: ignore
...
g = sns.catplot(
    ...
    kind='point', col_order=marker_names, row_order=freq_names)
...
for i_m, t_m in enumerate(marker_names):
    for i_f, t_f in enumerate(freq_names):
        ...
        stat_df.columns = stat_df.columns.levels[1]  # type: ignore
        ...
        ascending = bool(
            stat_df.mean()[-1] > stat_df.mean()[0])  # type: ignore
        ...
        t_ax = g.axes[i_f, i_m]
        x = t_ax.get_xticks()
        t_ax.errorbar(x, means.values, yerr=sems.values,  # type: ignore
...
means = stat_df.mean()
sems = stat_df.sem()
ascending = bool(stat_df.mean()[-1] > stat_df.mean()[0])  # type: ignore
print('Ascending' if ascending else 'Descending')
L, p = pageTest(stat_df, 'Subject', order, ascending=ascending)
text = f'L={L} \np={p:.2e}'
...
x = ax.get_xticks()
ax.errorbar(
    x, means.values, yerr=sems.values,  # type: ignore
    color='k', lw=2, zorder=100)
...
# Now plot differentes to awake:

diff_order = order[1:]

diff_df = first_df.copy()
diff_df = diff_df.set_index(
    ['Subject', 'Marker', 'Freq', 'Reduction']).unstack()
diff_df.columns = diff_df.columns.levels[1]  # type: ignore
for so in diff_order:
    diff_df[so] = diff_df[so] - diff_df['Awake']  # type: ignore

diff_df = diff_df[diff_order]
diff_df.columns = [f'{x}-W' for x in diff_df.columns]
diff_df.columns.name = 'Reduction'  # type: ignore
diff_df = diff_df.stack()
diff_df.name = 'Value'
diff_df = diff_df.reset_index()

diff_order = [f'{x}-W' for x in diff_order]
g = sns.catplot(
    x='Reduction', y='Value', col='Marker', row='Freq', hue='Subject',
    data=diff_df, legend=False, sharey=False, hue_order=subjects,
    order=diff_order, kind='point', col_order=marker_names,
    row_order=freq_names)
...
for i_m, t_m in enumerate(marker_names):
    for i_f, t_f in enumerate(freq_names):
        stat_df = diff_df.query(
            f"Marker == '{t_m}' and Freq == '{t_f}'")[
                ['Subject', 'Reduction', 'Value']].set_index(
                    ['Subject', 'Reduction']).unstack()
        stat_df.columns = stat_df.columns.levels[1]  # type: ignore
        stat_df = stat_df[diff_order]
        ...
        ascending = bool(
            stat_df.mean()[-1] > stat_df.mean()[0])  # type: ignore
        print('Ascending' if ascending else 'Descending')
        L, p = pageTest(stat_df, 'Subject', diff_order, ascending=ascending)
        text = f'L={L} \np={p:.2e}'
        print(f'{t_m} {t_f} {text}')
        if t_f == "Theta":
            pos = (0.25 + 0.31 * i_m, 0.86)
...
g.fig.savefig(f'../figures/stages/{prefix}_{plot_type}_stages_diff.pdf')
g.fig.savefig(f'../figures/stages/{prefix}_{plot_type}_stages_diff.png')
plt.close(g.fig)
